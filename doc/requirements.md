...
Frosted Window 是一个 `casual` 类型的移动端小游戏：Drag to wipe the steam — fast swipe = thin line, slow drag = wide brush, hold to fatten. Six scenes: vanity mirror, neon city, cabin, aquarium, sunset palms, blossom. Live drips + ambient pads.

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #fff、rgba(255,255,255,0.18)、rgba(255, 255, 255, 0.95)、#ffffff、rgba(255,255,255,0.7)、rgba(0,0,0,0.55)、rgba(0,0,0,0.4)、rgba(255,255,255,0.55)；高亮元素用于可点击目标、得分、结果或稀有状态。
